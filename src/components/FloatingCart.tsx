...
export default function FloatingCart() {
    ...
    return (
        <Link
            ...
            className={`fixed bottom-6 right-6 z-[100] h-14 bg-brand-900 text-white rounded-full shadow-2xl flex items-center justify-center cursor-pointer overflow-hidden border-2 border-white transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${expanded ? 'w-[280px] !rounded-[20px]' : 'w-14 rounded-full'}`}
        >
            ...
            {/* Standard UI (Icon + Badge) - Hidden when expanded */}
            <div className={`relative z-10 flex items-center justify-center w-full h-full transition-opacity duration-300 ${expanded ? 'opacity-0 hidden' : 'opacity-100'}`}>
                ...
                {totalQty > 0 && (
                    <span className="absolute -top-1 -right-1 bg-brand-500 text-white text-[9px] w-5 h-5 rounded-full flex items-center justify-center font-bold">
                        {totalQty}
                    </span>
                )}
            </div>
